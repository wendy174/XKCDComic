...
function JokeDetail() {
    ...
    if(joke.comments.length > 0) {
        maxCommentID.current = joke.comments[joke.comments.length - 1].id;
    }
    ...
    function handlePostComment(e) {
        e.preventDefault();
        maxCommentID.current = maxCommentID.current + 1;

        console.log(maxCommentID.current);

        const newCommentObj = {
            id: maxCommentID.current,
            text: newComment
        };
