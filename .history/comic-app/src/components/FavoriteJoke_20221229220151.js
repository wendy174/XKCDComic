...
function FavoriteJoke({ favorite, unfavoriteJoke }) {
    ...
    function unfavoriteHandler() {
        fetch(`http://localhost:3001/favorites/${favorite.id}`, { method: "DELETE"})
        .then(resp => resp.json())
        .then(data => {
            console.log(data);
        ...
        <>
            <div className="p-5 text-bg-dark rounded-3">
                ...
                <div className="gap-2 d-sm-flex">
                    <button className="btn btn-warning" onClick={ () => unfavoriteJoke(favorite.id) } type="button">Unfavorite</button>
